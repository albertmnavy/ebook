...
const version = '20260911_qr_storage';
const sql = await readFile(new URL('./migrations/20260911_qr_storage.sql', import.meta.url), 'utf8');
...
try {
  ...
  if (existing.rows[0]) {
    ...
    console.log('QR storage migration is already up to date.');
  } else {
    await client.query(sql);
    await client.query('INSERT INTO schema_migrations (version, checksum) VALUES ($1, $2)', [version, checksum]);
    await client.query('COMMIT');
    console.log('QR storage migration applied.');
  }
} catch (error) {
