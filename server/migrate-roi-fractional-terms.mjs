...
const version = '20260911_basic_roi_fractional_terms';
const sql = await readFile(new URL('./migrations/20260911_basic_roi_fractional_terms.sql', import.meta.url), 'utf8');
...
try {
  ...
  if (existing.rows[0]) {
    ...
    console.log('Basic fractional ROI terms migration is already up to date.');
  } else {
    await client.query(sql);
    await client.query('INSERT INTO schema_migrations (version, checksum) VALUES ($1, $2)', [version, checksum]);
    await client.query('COMMIT');
    console.log('Basic fractional ROI terms migration applied.');
  }
} catch (error) {
