...
function packagePage(type) {
  ...
  const title = isFD ? 'FD Package' : 'E-Book Packages'; const crumb = isFD ? 'Package / FD Package' : 'Package / E-Book Plans';
  return `${isFD ? '' : '<div class="basic-package-view">'}${pageHead(title, crumb, '')}<div class="package-header"><div class="available-fund">Available Fund Balance : <span>${displayMoney(state.availableBalance)}</span></div></div><div class="package-grid ${isFD ? '' : 'basic-package-grid'}">${cards}</div>${isFD ? '' : '</div>'}`;
}
