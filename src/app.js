...
function bookIcon() { return '<svg class="book-icon" viewBox="0 0 48 48" aria-hidden="true" focusable="false"><path d="M7 9.5A4.5 4.5 0 0 1 11.5 5H22a5 5 0 0 1 4 2 5 5 0 0 1 4-2h6.5A4.5 4.5 0 0 1 41 9.5v27a3.5 3.5 0 0 1-3.5 3.5H30a4 4 0 0 0-4 2 4 4 0 0 0-4-2h-7.5A7.5 7.5 0 0 1 7 32.5v-23Z" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linejoin="round"/><path d="M26 9v30M12 12h8M12 18h8M32 12h5M32 18h5" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>'; }
function loginPage() {
...
function packagePage(type) {
  ...
  return `${pageHead(isFD ? 'FD Package' : 'Basic Package', `Package / ${isFD ? 'FD Package' : 'Base Package'}`, '')}<div class="package-header"><div class="available-fund">Available Fund Balance : <span>${displayMoney(state.availableBalance)}</span></div></div><div class="package-grid">${items.map(item => isFD ? `<article class="package-card"><div class="package-card-header"><div class="package-icon">${bookIcon()}</div><h3>${item[0]}</h3></div><div class="package-details"><div class="detail-row"><span>Amount :</span><span>${item[1]}</span></div><div class="detail-row"><span>Days :</span><span>${item[2]}</span></div><div class="detail-row"><span>Total Return :</span><span>${item[3]}</span></div></div><button class="purchase-button" data-kind="FD" data-package="${item[0]}" data-amount="${item[1]}" data-days="${item[2]}" data-return="${item[3]}">Purchase</button></article>` : `<article class="package-card"><div class="package-card-header"><div class="package-icon">${bookIcon()}</div><h3>${item[0]}</h3></div><div class="package-details"><div class="detail-row"><span>Amount :</span><span>${item[1]}</span></div><div class="detail-row"><span>Daily ROI :</span><span>${item[2]}</span></div><div class="detail-row"><span>Days :</span><span>${item[3]}</span></div><div class="detail-row"><span>Total Return :</span><span>${item[4]}</span></div></div><button class="purchase-button" data-kind="Basic" data-package="${item[0]}" data-amount="${item[1]}" data-days="${item[3]}" data-return="${item[4]}">Purchase</button></article>`).join('')}</div>`;
}
