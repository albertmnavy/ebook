...
function rechargePage() {
  ...
  const minimumMinor = Number(settings.minimum_amount_minor); const maximumMinor = Number(settings.maximum_amount_minor); const hasMinimum = Number.isFinite(minimumMinor) && minimumMinor > 0; const hasMaximum = Number.isFinite(maximumMinor) && maximumMinor > 0; const minimumAmount = hasMinimum ? minimumMinor / 100 : 1; const maximumAmount = hasMaximum ? maximumMinor / 100 : null; const limits = [hasMinimum ? `Minimum: ₹ ${displayMoney(minimumAmount)}` : '', hasMaximum ? `Maximum: ₹ ${displayMoney(maximumAmount)}` : ''].filter(Boolean).join(' · '); const limitHint = limits ? `<span class="recharge-limit-note">${limits}</span>` : ''; const minAttribute = ` min="${minimumAmount}"`; const maxAttribute = hasMaximum ? ` max="${maximumAmount}"` : ''; const amountPlaceholder = hasMinimum ? `Enter amount (Min. ₹ ${displayMoney(minimumAmount)})` : 'Enter amount';
  return `${pageHead('Recharge', 'Package / Recharge')}<div class="panel qr-card"><h2>Scan QR to Pay</h2>${paymentInfo}<div class="gateway-label">Payment QR <span>Manual verification</span></div><div class="qr" id="qr" aria-label="Configured payment QR code"></div><div class="recharge-input"><label class="auth-label" for="recharge-amount">Recharge amount${limitHint}</label><input id="recharge-amount" class="input" type="number"${minAttribute}${maxAttribute} step="0.01" placeholder="${amountPlaceholder}"${disabled} /></div><div class="recharge-input"><label class="auth-label" for="payment-reference">UTR / payment reference</label><input id="payment-reference" class="input" maxlength="160" placeholder="Enter payment reference"${disabled} /></div><div class="center"><button class="primary-button" data-action="confirm-payment"${disabled}>Submit Recharge Request</button></div><div class="notice" style="margin:9px 0 0">Your request stays pending until the platform owner verifies the payment.</div></div><div class="section-gap"></div>${tablePanel('Payment History', ['SR', 'DATE', 'PAYMENT ID', 'METHOD', 'AMOUNT', 'STATUS'])}`;
}
...
function packagePage() {
  ...
  const cards = plans.length ? plans.map((plan, index) => {
    ...
    const details = `<div class="package-details"><div class="detail-row"><span>Amount :</span><span>${displayMoney(Number(plan.amount_minor) / 100)}</span></div><div class="detail-row"><span>Daily ROI :</span><span>${displayMoney(Number(plan.daily_roi_minor) / 100)}</span></div><div class="detail-row"><span>Days :</span><span>${plan.duration_days}</span></div><div class="detail-row"><span>Total ROI :</span><span>${displayMoney(Number(plan.total_return_minor) / 100)}</span></div></div>`;
...
async function submitRecharge() {
  ...
  const settings = app.paymentSettings || {}; const minimumMinor = Number(settings.minimum_amount_minor); const maximumMinor = Number(settings.maximum_amount_minor); const amountMinor = Math.round(amount * 100);
  if (Number.isFinite(minimumMinor) && amountMinor < minimumMinor) return authError(`Recharge amount must be at least ₹ ${displayMoney(minimumMinor / 100)}.`);
  if (Number.isFinite(maximumMinor) && maximumMinor > 0 && amountMinor > maximumMinor) return authError(`Recharge amount must not exceed ₹ ${displayMoney(maximumMinor / 100)}.`);
